...
### 1-minute data + daily/weekly rebalancing

Hermis can backtest on 1-minute bars while still rebalancing on a slower clock (daily/weekly/monthly). Use:

- `configs/nifty500_ema_online.yaml` (defaults to 1min)
- `configs/nifty500_ema_online_1D.yaml` (daily bars)

**Lookback note:** in configs, `risk_model.lookback` now supports a `lookback_unit` so the same config can work on both 1D and 1min data. For example, `lookback: 90` with `lookback_unit: trading_days` becomes ~90×(bars_per_day) bars on minute data.

By default, when running intraday data, Hermis scales mean/cov estimates to the rebalance horizon (log-return approximation). You can turn this off with `risk_model.scale_to_rebalance_horizon: false`.
