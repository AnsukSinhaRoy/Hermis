...
def get_rebalance_dates(prices: pd.DataFrame, freq: str = "monthly") -> pd.DatetimeIndex:
    ...
    if freq == "daily":
        # For intraday data, "daily" should mean once per day (first available bar).
        # For daily data, this is identical (one bar per day).
        day_labels = idx.normalize()
        picks = (
            pd.Series(idx)
            .groupby(day_labels)
            .min()
...
def run_backtest(
    ...
    # --- Constraints (used by backtest bookkeeping; optimizer may enforce more) ---
    opt_cfg = config.get('optimizer', {}) if isinstance(config.get('optimizer', {}), dict) else {}
    constraints_cfg = config.get('constraints', {}) if isinstance(config.get('constraints', {}), dict) else {}
    long_only = bool(constraints_cfg.get('long_only', opt_cfg.get('long_only', True)))
    ...
    rebalance_mask = pd.Index(dates).isin(rebalance_dates)
    ...
    def _apply_cash_overlay(w: pd.Series) -> pd.Series:
        ...
        # Static cap (primarily for long-only portfolios).
        ...
        if long_only:
            ...
            # Cash is the residual (clamped to [0, 1] in long-only mode).
            cash_w = 1.0 - risky_sum
            cash_w = float(max(0.0, min(1.0, cash_w)))
        else:
            # Short-enabled mode: treat cash as residual of net exposure. Do NOT clamp.
            cash_w = 1.0 - risky_sum
    ...
    # --- Main backtest loop ---
    # Track the previous rebalance timestamp so online optimizers (OMD/FTRL)
    # can update using the full holding-period return rather than only the last bar.
    last_rebalance_i = 0
    ...
    for i, date in enumerate(dates):
        ...
        # --- Rebalancing Logic ---
        if bool(rebalance_mask[i]):
            day_did_rebalance = True

            past_prices = prices.iloc[:i]
            ...
            # Holding-period window since the previous rebalance (strictly before `date`),
            # useful for online optimizers to update on the realized return of the period.
            period_prices = prices.iloc[max(0, int(last_rebalance_i)) : i]
            period_prices_risky = period_prices.drop(columns=[cash_name], errors='ignore') if cash_enabled else period_prices
            ...
            if len(common_assets) == 0:
                ...
                try:
                    ...
                    opt_result = optimizer_func(
                        er,
                        cov,
                        prices_window=past_prices_risky,
                        period_prices_window=period_prices_risky,
                        date=date,
                        prev_date=prev_date,
            ...
            if target_weights is None:
                ...
                if cash_enabled and cash_name in prices.columns:
                    valid_assets = prices.loc[date].drop(labels=[cash_name], errors='ignore').dropna().index.tolist()
                else:
            ...
            # Update last rebalance marker even if optimizer fell back.
            last_rebalance_i = i
