...
def compute_drawdown(nav_s: pd.Series) -> pd.DataFrame:
    # nav_s is cumulative NAV series (index = dates)
    roll_max = nav_s.cummax()
    drawdown = (nav_s - roll_max) / roll_max
    dd_df = pd.DataFrame({"nav": nav_s, "peak": roll_max, "drawdown": drawdown})
    return dd_df

def rolling_sharpe(returns: pd.Series, window: int = 63) -> pd.Series:
    # returns: simple or log returns (we expect simple pct returns)
    # approximate: mean/std * sqrt(252)
    r = returns.rolling(window).mean()
    s = returns.rolling(window).std()
    rs = (r / s) * np.sqrt(252)
    return rs

def ann_stats(nav_s: pd.Series) -> Dict[str, float]:
    # annualized return, vol, max drawdown
    ...
    # max drawdown
    dd_df = compute_drawdown(nav_s)
    max_dd = float(dd_df["drawdown"].min())
    return {"total_return": total_ret, "ann_return": ann_ret, "ann_vol": float(ann_vol), "max_drawdown": max_dd}
...
sample_max = st.sidebar.number_input("Max assets to draw (prices tab)", min_value=10, max_value=2000, value=200, step=10)
...
with tabs[0]:
    ...
    with col1:
        ...
        if nav is not None:
            ...
            st.metric("Annualized return", f"{stats['ann_return']*100:.2f} %")
            st.metric("Annualized vol", f"{stats['ann_vol']*100:.2f} %")
    ...
    with col2:
        ...
        if prices is None:
            st.info("No saved prices snapshot found in experiment/data. Prices tab may regenerate or be unavailable.")
...
with tabs[4]:
    ...
    if nav is None:
        ...
        nav_s = ensure_series(nav)
        cum = cumulative_returns(nav_s)
        dr = nav_s.pct_change().dropna()
        # cumulative returns plot
        fig = px.line(x=cum.index, y=cum.values, title="Cumulative NAV (start=1)", labels={"x":"date","y":"cum_return"})
        st.plotly_chart(fig, use_container_width=True)
        # drawdown
        dd = compute_drawdown(nav_s)
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=dd.index, y=dd['drawdown'], fill='tozeroy', name='Drawdown'))
        ...
        fig3 = px.line(x=rs.index, y=rs.values, title=f"Rolling Sharpe (window={win} days)")
    ...
    if prices is None:
        ...
        if chosen:
            ...
            if weights is not None:
                # show weight time series for each chosen asset
                wsub = weights[chosen].copy()
                # downsample markers for display
            ...
            st.plotly_chart(fig, use_container_width=True)
            # allow CSV download of chosen price subset
...
with tabs[6]:
    ...
    # If comparison not enabled, ask user to enable
    if not cmp_paths:
        st.info("Enable comparison mode in the sidebar to pick experiments.")
    else:
        # Helper metric functions (local to this block)
        ...
        # Pick experiments, compute metrics
        metric_rows = []
        nav_series_dict = {}
        for p in cmp_paths:
